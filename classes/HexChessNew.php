...
class HexChess {
    ...
    public function __construct($gameId = null, $playerCount = 2, $boardSize = 8) {
        ...
        $this->playerCount = max(2, min(6, $playerCount)); // Updated to support up to 6 players
...
private function canPawnMove($piece, $fromQ, $fromR, $toQ, $toR) {
    ...
    if ($this->playerCount == 2) {
        if ($piece->player == 0) { // Red player (West edge)
            if ($dq == 1 && $dr == 0) {
                return !$this->getPiece($toQ, $toR);
            }
            // Captures: (0, 1) and (1, -1)
            if (($dq == 0 && $dr == 1) || ($dq == 1 && $dr == -1)) {
                $targetPiece = $this->getPiece($toQ, $toR);
                return $targetPiece && $targetPiece->player !== $piece->player;
            }
        } else { // Blue player (East edge)
            if ($dq == -1 && $dr == 0) {
                return !$this->getPiece($toQ, $toR);
            }
            // Captures: (0, -1) and (-1, 1)
    ...
    } elseif ($this->playerCount == 3) {
        switch ($piece->player) {
            case 0: // Red (West) - forward (1, 0)
                if ($dq == 1 && $dr == 0) {
                    return !$this->getPiece($toQ, $toR);
                }
                // Captures: (0, 1) and (1, -1)
            ...
            case 1: // Purple (Northeast) - forward (-1, 1)
                if ($dq == -1 && $dr == 1) {
                    return !$this->getPiece($toQ, $toR);
                }
                // Captures: (-1, 0) and (0, 1)
                if (($dq == -1 && $dr == 0) || ($dq == 0 && $dr == 1)) {
                    $targetPiece = $this->getPiece($toQ, $toR);
                    return $targetPiece && $targetPiece->player !== $piece->player;
                }
                break;
                
            case 2: // Blue (Southeast) - forward (0, -1)
                if ($dq == 0 && $dr == -1) {
                    return !$this->getPiece($toQ, $toR);
                }
                // Captures: (1, -1) and (-1, 0)
                if (($dq == 1 && $dr == -1) || ($dq == -1 && $dr == 0)) {
                    $targetPiece = $this->getPiece($toQ, $toR);
                    return $targetPiece && $targetPiece->player !== $piece->player;
                }
                break;
...
/*private function canPawnMove($piece, $fromQ, $fromR, $toQ, $toR) {
    ...
    // Debug for red pawn in 3-player games
    if ($this->playerCount == 3 && $piece->player == 0) {
        error_log("RED PAWN: testing move ($dq,$dr) from ($fromQ,$fromR) to ($toQ,$toR)");
    }
    
    // Get king's starting position
    $kingStartQ = 0;
    $kingStartR = 0;
    $this->getKingStartingPosition($piece->player, $kingStartQ, $kingStartR);
    
    $forwardDq = 0;
    $forwardDr = 0;
    $captureDirections = [];
    
    // Determine direction based on king's STARTING position on the board
    if ($kingStartQ == -$this->boardSize && $kingStartR == 0) {
        // West edge king: move East
        $forwardDq = 1; $forwardDr = 0;
        $captureDirections = [[0, 1], [1, -1]];
        if ($this->playerCount == 3 && $piece->player == 0) {
            error_log("RED PAWN: West edge - forward=(1,0), captures=[(0,1), (1,-1)]");
        }
        
    } elseif ($kingStartQ == $this->boardSize && $kingStartR == 0) {
        // East edge king: move West  
        $forwardDq = -1; $forwardDr = 0;
        $captureDirections = [[0, -1], [-1, 1]];
        
    } elseif ($kingStartQ == 0 && $kingStartR == -$this->boardSize) {
        // Northwest corner king: move Southeast
        $forwardDq = 0; $forwardDr = 1;
        $captureDirections = [[-1, 1], [1, 0]];
        
    } elseif ($kingStartQ == $this->boardSize && $kingStartR == -$this->boardSize) {
        // Northeast corner king: move Southwest
        $forwardDq = -1; $forwardDr = 1;
        $captureDirections = [[-1, 0], [0, 1]];
        
    } elseif ($kingStartQ == 0 && $kingStartR == $this->boardSize) {
        // Southeast corner king: move Northwest
        $forwardDq = 0; $forwardDr = -1;
        $captureDirections = [[1, -1], [-1, 0]];
        
    } elseif ($kingStartQ == -$this->boardSize && $kingStartR == $this->boardSize) {
        // Southwest corner king: move Northeast
        $forwardDq = 1; $forwardDr = -1;
        $captureDirections = [[0, -1], [1, 0]];
        
    } else {
        // Fallback for any other king positions
        if ($this->playerCount == 3 && $piece->player == 0) {
            error_log("RED PAWN ERROR: Unknown king starting position: ($kingStartQ, $kingStartR)");
        }
        return false;
    }
    
    // Check forward movement (no capture)
    if ($dq == $forwardDq && $dr == $forwardDr) {
        $result = !$this->getPiece($toQ, $toR);
        if ($this->playerCount == 3 && $piece->player == 0) {
            error_log("RED PAWN: forward move check - " . ($result ? "VALID (empty square)" : "BLOCKED (occupied)"));
        }
        return $result;
    }
    
    // Check diagonal captures
    foreach ($captureDirections as $captureDir) {
        if ($dq == $captureDir[0] && $dr == $captureDir[1]) {
            $targetPiece = $this->getPiece($toQ, $toR);
            if ($this->playerCount == 3 && $piece->player == 0) {
                if ($targetPiece) {
                    $valid = ($targetPiece->player !== $piece->player);
                    error_log("RED PAWN: capture direction [$captureDir[0],$captureDir[1]] - found {$targetPiece->type} player {$targetPiece->player} - " . ($valid ? "VALID CAPTURE" : "SAME PLAYER - INVALID"));
                    return $valid;
                } else {
                    error_log("RED PAWN: capture direction [$captureDir[0],$captureDir[1]] - empty square - INVALID CAPTURE");
                    return false;
                }
            }
            return $targetPiece && $targetPiece->player !== $piece->player;
    ...
    if ($this->playerCount == 3 && $piece->player == 0) {
        error_log("RED PAWN: move ($dq,$dr) doesn't match any valid pattern");
    }
    return false;
}
*/
...
private function canPawnMove($piece, $fromQ, $fromR, $toQ, $toR) {
    ...
    if ($this->playerCount == 2) {
        ...
        switch ($piece->player) {
            ...
            case 1: // Blue (Northeast) - forward (-1, 1)
                if ($dq == -1 && $dr == 1) {
                    return !$this->getPiece($toQ, $toR);
                }
                // Captures: (-1, 0) and (0, 1)
            ...
            case 2: // Green (Southeast) - forward (0, -1)
                if ($dq == 0 && $dr == -1) {
                    return !$this->getPiece($toQ, $toR);
                }
                // Captures: (1, -1) and (-1, 0)
        ...
        switch ($piece->player) {
            ...
            case 1: // Green (Northeast) - forward (-1, 1)
                if ($dq == -1 && $dr == 1) {
                    return !$this->getPiece($toQ, $toR);
                }
                // Captures: (-1, 0) and (0, 1)
            ...
            case 2: // Yellow (East) - forward (-1, 0)
                if ($dq == -1 && $dr == 0) {
                    return !$this->getPiece($toQ, $toR);
                }
                // Captures: (0, -1) and (-1, 1)
            ...
            case 3: // Orange (Southwest) - forward (1, -1)
                if ($dq == 1 && $dr == -1) {
                    return !$this->getPiece($toQ, $toR);
                }
                // Captures: (0, -1) and (1, 0)
        ...
        switch ($piece->player) {
            ...
            case 1: // Blue (Northwest) - forward (0, 1)
                if ($dq == 0 && $dr == 1) {
                    return !$this->getPiece($toQ, $toR);
                }
                // Captures: (-1, 1) and (1, 0)
            ...
            case 2: // Green (Northeast) - forward (-1, 1)
                if ($dq == -1 && $dr == 1) {
                    return !$this->getPiece($toQ, $toR);
                }
                // Captures: (-1, 0) and (0, 1)
            ...
            case 3: // Yellow (East) - forward (-1, 0)
                if ($dq == -1 && $dr == 0) {
                    return !$this->getPiece($toQ, $toR);
                }
                // Captures: (0, -1) and (-1, 1)
            ...
            case 4: // Purple (Southeast) - forward (0, -1)
                if ($dq == 0 && $dr == -1) {
                    return !$this->getPiece($toQ, $toR);
                }
                // Captures: (1, -1) and (-1, 0)
        ...
        switch ($piece->player) {
            ...
            case 5: // Orange (Southwest) - forward (1, -1)
                if ($dq == 1 && $dr == -1) {
                    return !$this->getPiece($toQ, $toR);
                }
                // Captures: (0, -1) and (1, 0)
...
private function getKingStartingPosition($player, &$startQ, &$startR) {
    // Determine king's starting position based on player slot and current game setup
    switch ($this->playerCount) {
        case 2:
            if ($player == 0) {
                // Red - West edge
                $startQ = -$this->boardSize;
                $startR = 0;
            } else {
                // Purple - East edge  
                $startQ = $this->boardSize;
        ...
        case 3:
            if ($player == 0) {
                ...
                // Purple - Northeast corner
                $startQ = $this->boardSize;
                $startR = -$this->boardSize;
            } else {
        ...
        case 5:
            if ($player == 0) {
                ...
                // Purple - Northwest corner
                $startQ = 0;
                $startR = -$this->boardSize;
            } elseif ($player == 2) {
                // Blue - Northeast corner
                $startQ = $this->boardSize;
                $startR = -$this->boardSize;
            } elseif ($player == 3) {
                // Green - East edge
