...
// Clean checkmate display functions - add to game.js

function parseCheckmateMessage(statusText) {
    const playerNames = ['red', 'blue', 'green', 'yellow', 'purple', 'bronze'];
    
    for (let i = 0; i < playerNames.length; i++) {
        if (statusText.includes(playerNames[i] + ' wins by checkmate')) {
            const playerName = playerNames[i].charAt(0).toUpperCase() + playerNames[i].slice(1);
            return `${playerName} wins by checkmate!`;
        }
    }
...
function showEnhancedGameOver(message, type, gameEndType) {
    console.log('Game over:', message, type, gameEndType);
    
    const gameStatus = document.getElementById('gameStatusText');
    if (gameStatus) {
        // Simple white text display, no colors or fancy styling
        gameStatus.innerHTML = `<div style="
            font-size: 18px; 
            font-weight: bold; 
            color: white;
    ...
    // Simple notification without emojis
    showNotification('Game Complete', message, 'info');
}
